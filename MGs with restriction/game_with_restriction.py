...
for avg in range(total_avgs):
    ...
    for n in range(max_iterations):
        ...
        p = 0
        s_new1 = -1
        while (p < p_s_new) and (s_new1 < (s_new - 1)):
            s_new1 = s_new1 + 1
            #print(a1,a2,s,s_new)
        ...
        delta = (r + discount * next_state_value + discount * (100/ (n+100) ) * (r1 + discount * next_state_value1)- Q[state, act1,act2]) 
        #dQ = (1 / math.sqrt(np.sum(tot_count[act1][act2][state]))) * delta 
        dQ = (1 / (np.sum(tot_count[act1][act2][state]))**0.501) * delta  
#         dQ = 0.9 * delta 
        Q[state, act1,act2] += dQ
...
plt.savefig('Final_average10_5.pdf', dpi=600, bbox_inches='tight')
plt.close()
"""